...
class Terrain:
    def __init__(self, width, height):
        ...
        self.height = height
        self.pickupItems =[]
        self.placeItemPickUP(5)


    def genRandomPoint(self):
        x = Vector(random.randrange(1200),
    ...
    def placeItemPickUP(self, maxItems): # skeleton code for placing item pickups. NEEDS FIXING CURRENTLY LOOPS ROUND INFI
        r = random
        """xPosPickUp = r.randrange(1200)
        yPosPickUp = r.randrange(800)
        xYOfPickUp = Vector(random.randrange(1200),
                        random.randrange(800))"""
        itemRadius = 10
        canPlaceItem = True
        count = 0
        for i in range(0, maxItems):# for each coordinate
            xYOfPickUp = self.genRandomPoint()
            for line in self.lines:
                if line.distanceTo(xYOfPickUp) < itemRadius*2:
                    canPlaceItem = False
                else:
                    canPlaceItem = True

            if canPlaceItem == True:
                self.pickupItems.append(xYOfPickUp)
                print("canPlaceItem", canPlaceItem)
                count = count + 1
                print("count: ", count, ". MaxItems: ", maxItems)
    ...
    def drawItemPickUp(self, canvas):
        ...
        for item in self.pickupItems:
            ...
            canvas.draw_circle((item.x, item.y), 10, LINE_WIDTH, 'Red', 'Red')

        return len(self.pickupItems)
